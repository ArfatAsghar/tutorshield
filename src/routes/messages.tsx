...
import { ShieldAlert, Send, Sparkles, Loader2, MessageSquare, ArrowLeft, ChevronRight } from "lucide-react";
...
// Mobile: either 'inbox' or 'chat' view
...
function Messages() {
  ...
  // Mobile view state — starts at inbox, goes to chat when contact selected
  ...
  // ──────────────────────────────────────────────────
  // INBOX PANEL (shared between mobile and desktop)
  // ──────────────────────────────────────────────────
  const InboxPanel = () => (
          ...
          contacts.map((c) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-4 flex gap-3 items-center transition-colors hover:bg-muted/30 ${
                  isActive ? "bg-muted/50 border-l-4 border-primary" : ""
                }`}
              >
                <img src={c.avatar} className="w-11 h-11 rounded-full bg-muted shrink-0" alt="" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5 ml-2 shrink-0">
                      ...
                      {/* Mobile chevron hint */}
                      <ChevronRight className="w-4 h-4 text-muted-foreground/50 md:hidden" />
                    </div>
                  </div>
  ...
  const ChatPanel = () => (
    activeContact ? (
      <div className="flex flex-col h-full">
        ...
        <div className="border-b border-border p-4 flex items-center justify-between bg-card shrink-0">
          <div className="flex items-center gap-3">
            {/* Back arrow for mobile */}
            <button
              onClick={handleBackToInbox}
              className="md:hidden -ml-1 p-1.5 rounded-lg hover:bg-muted/50 transition-colors"
          ...
          <Badge className="hidden md:flex bg-accent/20 text-accent-foreground gap-1">
        ...
        {/* Send Form */}
        <form onSubmit={send} className="border-t border-border p-3 flex gap-2 bg-card shrink-0">
          <Input
            value={text}
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-8 space-y-2 text-center h-full">
        ...
        <p className="text-xs max-w-xs">Tap any conversation from the inbox to start chatting.</p>
      </div>
    )
  );
  ...
  return (
    <div className="space-y-4">
      {/* Header — only show on inbox on mobile or always on desktop */}
      <div className={`${mobileView === "chat" ? "hidden md:flex" : "flex"} items-center justify-between flex-wrap gap-2`}>
        <div>
          <h1 className="text-3xl font-bold">Messages</h1>
          <p className="text-muted-foreground mt-1">Chat securely. Powered by WebSockets.</p>
        </div>
      ...
      <div className="hidden md:block">
        <Card className="overflow-hidden border border-border shadow-glow">
          <div className="grid md:grid-cols-3" style={{ height: "560px" }}>
            {/* Left: Inbox */}
            <div className="border-r border-border flex flex-col bg-card/50 overflow-hidden">
              <InboxPanel />
            </div>
            {/* Right: Chat */}
            <div className="md:col-span-2 flex flex-col bg-card/20 overflow-hidden">
              <ChatPanel />
            </div>
          </div>
        </Card>
      ...
      {/* ── MOBILE: single panel, switches between inbox and chat ── */}
      <div className="md:hidden">
        <Card className="overflow-hidden border border-border" style={{ height: "calc(100vh - 180px)" }}>
          {mobileView === "inbox" ? (
      ...
      {/* Safety Warning — only show in inbox mode on mobile */}
      <Card className={`border-warning/40 bg-warning/5 ${mobileView === "chat" ? "hidden md:block" : ""}`}>
